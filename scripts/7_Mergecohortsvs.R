...
#directorypatients <- "/home/user/MARTA/LABO/PULPO/PULPO_synthetic/results/SVs/Patients/"
#directorypatients <-"/home/user/MARTA/LABO/PULPO/PULPO_synthetic/results/SVs/Patients/Syntheticpatient2/SigProfiler/results/MatrixGenerator/"
#########################################
...
for (i in patients) {
  fulldirectory <- paste0(directorypatients, "/", i, "/SigProfiler/results/MatrixGenerator/", i, ".SV32.matrix.tsv")
...
for (j in directories) {
  if (file.exists(j)) {
    ...
    warning(paste("The archive", j, "does not exist"))
  }
 # file <- read.table(j, header = TRUE)
